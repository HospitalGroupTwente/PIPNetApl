...
def train_pipnet(net, train_loader, optimizer_net, optimizer_classifier, scheduler_net, scheduler_classifier, criterion,
    ...
    if pretrain:
        ...
        progress_prefix = 'Pretrain Epoch'  # TODO: add another flag
    else:
...
def calculate_loss(proto_features, pooled, out, ys1, align_pf_weight, t_weight, unif_weight, cl_weight,
                   net_normalization_multiplier, pretrain, finetune, criterion, train_iter, print=True, EPS=1e-10):
...
def train_pipnet_furhter(net, train_loader, optimizer_net, optimizer_classifier, scheduler_net, scheduler_classifier,
                         criterion, epoch, nr_epochs, device, only_classifier=False, finetune=False,
    ...
    if only_classifier:
        # Disable training of classification layer
        # net.module._net.requires_grad = False
        # net.module._add_on.requires_grad = False
        # net.module._pool.requires_grad = False
        # net.module._classification.requires_grad = True
        progress_prefix = 'Only Classifier'  # TODO: add another flag
    else:
        # Enable training of classification layer (disabled in case of pretraining)
        net.module._classification.requires_grad = True
    ...
    count_param = 0
    for name, param in net.named_parameters():
        # print(f"named parameters: {name}")
        if param.requires_grad:
            count_param += 1
    print("Number of parameters that require gradient: ", count_param, flush=True)

    # TODO: check that the weights are ok
    align_pf_weight = 5.
    t_weight = 2.
    unif_weight = 0.
    cl_weight = 2.
    ...
    print("Align weight: ", align_pf_weight, ", U_tanh weight: ", t_weight, "Class weight:", cl_weight, flush=True)
    print("Only Classifier?", only_classifier, "Finetune?", finetune, flush=True)
    ...
    for i, (xs1, xs2, ys) in train_iter:
        ...
        proto_features, pooled, out = net(torch.cat([xs1, xs2]))
        loss, acc = calculate_loss(proto_features, pooled, out, ys, align_pf_weight, t_weight, unif_weight, cl_weight,
                                   net.module._classification.normalization_multiplier, False, finetune, criterion,
                                   train_iter, print=True, EPS=1e-8)
