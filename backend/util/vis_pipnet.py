...
def visualize_topk(net, projectloader, num_classes, device, foldername, args: argparse.Namespace, k=10):
    ...
    print("patchsize: ", patchsize, "skip: ", skip, flush=True)
                        ...
                        if idx == i:
                            ...
                            with torch.no_grad():
                                softmaxes, pooled, out = net(xs,
                                                             inference=True)  # softmaxes has shape (1, num_prototypes, W, H)
                                outmax = torch.amax(out, dim=1)[
                                    0]  # shape ([1]) because batch size of projectloader is 1
                                if outmax.item() == 0.:
    ...
    for p in range(net.module._num_prototypes):
        if saved[p] > 0:
            try:
                ...
                pass
    # grid = torchvision.utils.make_grid(all_tensors, nrow=k, padding=1) # TODO: fix this error instead of just commenting it out
