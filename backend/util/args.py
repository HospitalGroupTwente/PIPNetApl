...
def get_args() -> argparse.Namespace:
    ...
    parser.add_argument('--validation_size',
                        type=float,
                        default=0.,
                        help='Split between training and validation set. Can be zero when there is a separate test or '
                             'validation directory. Should be between 0 and 1. Used for partimagenet (e.g. 0.2)')
    parser.add_argument('--net',
                        type=str,
                        default='convnext_tiny_26',
                        help='Base network used as backbone of PIP-Net. Default is convnext_tiny_26 with adapted '
                             'strides to output 26x26 latent representations. Other option is convnext_tiny_13 that '
                             'outputs 13x13 (smaller and faster to train, less fine-grained). Pretrained network on '
                             'iNaturalist is only available for resnet50_inat. Options are: resnet18, resnet34, '
                             'resnet50, resnet50_inat, resnet101, resnet152, convnext_tiny_26 and convnext_tiny_13.')
    parser.add_argument('--batch_size',
                        type=int,
                        default=64,
                        help='Batch size when training the model using minibatch gradient descent. Batch size is '
                             'multiplied with number of available GPUs')
    ...
    parser.add_argument('--epochs_pretrain',
                        type=int,
                        default=10,
                        help='Number of epochs to pre-train the prototypes (first training stage). Recommended to '
                             'train at least until the align loss < 1'
                        )
    parser.add_argument('--epochs_posttrain',
                        type=int,
                        default=10,
                        help='Number of epochs to unlearn the shortcuts in the prototypes (1.5 training stage) TODO:. '
    ...
    parser.add_argument('--num_features',
                        type=int,
                        default=0,
                        help='Number of prototypes. When zero (default) the number of prototypes is the number of '
                             'output channels of backbone. If this value is set, then a 1x1 conv layer will be added. '
                             'Recommended to keep 0, but can be increased when number of classes > num output '
                             'channels in backbone.')
    parser.add_argument('--image_size',
                        type=int,
                        default=224,
                        help='Input images will be resized to --image_size x --image_size (square). Code only tested '
                             'with 224x224, so no guarantees that it works for different sizes.')
    parser.add_argument('--state_dict_dir_net',
                        type=str,
                        default='',
                        help='The directory containing a state dict with a pretrained PIP-Net. E.g., '
                             './runs/run_pipnet/checkpoints/net_pretrained')
    parser.add_argument('--freeze_epochs',
                        type=int,
                        default=10,
                        help='Number of epochs where pretrained features_net will be frozen while training '
                             'classification layer (and last layer(s) of backbone)'
                        )
    ...
    parser.add_argument('--weighted_loss',
                        action='store_true',
                        help='Flag that weights the loss based on the class balance of the dataset. Recommended to '
    ...
    parser.add_argument('--bias',
                        action='store_true',
                        help='Flag that indicates whether to include a trainable bias in the linear classification '
                             'layer.'
                        )
    parser.add_argument('--extra_test_image_folder',
                        type=str,
                        default='./experiments',
                        help='Folder with images that PIP-Net will predict and explain, that are not in the training '
                             'or test set. E.g. images with 2 objects or OOD image. Images should be in folder. E.g. '
                             './experiments/images/')
    parser.add_argument('--force',
                        action='store_true',
                        help='Flag that indicates whether the run folder can be overwritten'
                        )
    parser.add_argument('--mlflow_experiment',
                        type=str,
                        default="pipnet-experiment",
                        help="Flag that determines under which experiment name the mlflow statistics are saved"
                        )
    parser.add_argument('--vis_dir',
                        type=str,
                        default="/mnt/data/ffobbe/data/hip/big_rectangle/trainval/",
                        help="The folder that contains the images corresponding to the shortcuts")
    parser.add_argument('--csv_name',
                        type=str,
                        default="mask_no_finding.csv",
                        help='name of the csv file that contains the shortcut positions in the image')
    parser.add_argument('--rect_dir',
                        type=str,
                        default="/mnt/data/ffobbe/data/hip/big_rectangle/trainval",
                        help='folder with the images that contain shortcuts')
    parser.add_argument('--norm_dir',
                        type=str,
                        default="/mnt/data/ffobbe/data/hip/normal/trainval",
                        help='folder with the images that do not contain shortcuts')
    args = parser.parse_args()
    if args.dataset == 'CLIPART_OR':
        args.bias = True
